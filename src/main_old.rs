...
use bevy::app::{AppLabel, FixedMain, MainSchedulePlugin, ScheduleRunnerPlugin, SubApp};
...
fn main2() {
    ...
    app
        ...
        // .insert_resource(PixelPositions {
        //     map: Default::default(),
        //     is_map_dirty: false,
    ...
    sub_app
        ...
        // .add_systems(Startup, create_pixels)
        // .add_systems(Main, || {
        //     // Piss
        // })
        .add_systems(Startup, create_pixels)
        .add_systems(FixedUpdate, sand_movement)
        .add_systems(Update, water_movement)
        .add_systems(Update, lava_movement);

    app.insert_sub_app(SandPosUpdatesApp, SubApp::new(sub_app, |main_world, sub_app| {
        // extract the value from the main app to the sub app
        sub_app.world.resource_mut::<PixelPositions>().map = main_world.resource_mut::<PixelPositions>().clone().map;
    }));
